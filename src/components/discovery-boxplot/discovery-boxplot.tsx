...
export class DiscoveryBoxPlotComponent {
    ...
    convert(data: DataModel) {
        ...
        for (let i = 0; i < gtsCount; i++) {
            ...
            */
            if (GTSLib.isGtsToPlot(gts) && !!gts.v) {
                this.isGTS = true;
                min = Math.min(gts.v.map(v => v[0]));
                max = Math.max(gts.v.map(v => v[0]));
                ...
                (opts.series as any[]).push({
                    ...
                    type: 'boxplot',
                    name,
                    data: [{value: [min, q1, median, q3, max], name}]
                } as SeriesOption);
            } else if (!gts.v) {
                ...
                (gts.columns ?? []).forEach((label: any, index: number) => {
                    ...
                    let type = ((data.params || [])[index] || {type: 'boxplot'}).type || 'boxplot';
                    let areaStyle;
                    if (type === 'area') {
                        type = 'line';
                        areaStyle = {
        ...
        opts.yAxis = {
            name: this.unit, // !this.innerOptions?.bar?.horizontal ? this.unit || this.innerOptions.unit : undefined,
            show: !this.innerOptions.hideYAxis,
            emphasis: {focus: 'series'},
            nameTextStyle: {color: Utils.getLabelColor(this.el)},
            type: 'value', //this.innerOptions?.bar?.horizontal ? 'category' : 'value',
        ...
        opts.xAxis = {
            ...
            emphasis: {focus: 'series'},
            type:'category',// this.innerOptions?.bar?.horizontal? 'value': 'category',
            axisLine: {lineStyle: {color: Utils.getGridColor(this.el)}},
            axisLabel: {
                show: !this.innerOptions.hideXAxis,
